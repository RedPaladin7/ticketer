...
const EventCard = ({ eventId }: { eventId: Id<"events"> }) => {
  ...
  const renderQueuePosition = () => {
    ...
    if(avaibility.purchasedCount >= avaibility.totalTickets){
        return (
            ...
            <div className="flex flex-col lg:flex-row items-center justify-between p-3 bg-amber-50 rounded-lg border border-amber-100">
                <div className="flex items-center">
                    ...
                    <span className="text-amber-700 font-medium">
                        Your&apos;re next in line! (Queue position:{' '})
  ...
  return (
    ...
    className={`bg-white rounded-xl shadow-sm hover:shadow-lg transition-all duration-300 border border-gray-100 cursor-pointer overflow-hidden relative ${
        isPastEvent ? 'opacity-75 hover:opactiy-100' : ''
    }`}
    >
        {imageUrl && (
        ...
        <div className={`p-6 ${imageUrl ? 'relative' : ''}`}>
            <div className="flex justify-between items-start">
                ...
                <div className="flex flex-col items-end gap-2 ml-4">
                    ...
                    {avaibility.purchasedCount >= avaibility.totalTickets && (
            ...
            <div className="mt-4 space-y-3">
                ...
                <div className="flex items-center text-gray-600">
                    ...
                    <span>
                        {avaibility.totalTickets - avaibility.purchasedCount} / {''}
                        {avaibility.totalTickets} available
                        {!isPastEvent && avaibility.activeOffers > 0 && (
                            <span className="text-amber-600 text-sm ml-2">
                                ({avaibility.activeOffers}{''}
                                {avaibility.activeOffers === 1 ? 'person' : 'people'} trying to buy)
                            </span>
                        )}
                    </span>
                </div>
